...
class OperationRetrier<T> {
    ...
    public static final int DEFAULT_NUMBER_OF_RETRIES = 60
    public static final int DEFAULT_DELAY_BETWEEN_RETRIES_IN_MILLIS = 5000
        ...
        while (true) {
            try {
                ...
                if (counter >= numberOfAttempts) {
                    ...
                    if (counter == 1) {
                        log.info("Requested operation wasn't successful in first try. ${formatMaximumRetryingPeriodMessage()}.")
                    }
